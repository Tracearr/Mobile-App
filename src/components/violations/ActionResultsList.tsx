 ...
 * Shows success/failure/skipped status for each action. The caller supplies the heading.
 */
  ...
  return (
    <View className="gap-1.5">
      {results.map((result, index) => (
        <View key={index} className="flex-row flex-wrap items-center gap-2">
          {result.skipped ? (
            <SkipForward size={16} color={colors.icon.default} />
          ) : result.success ? (
          ...
          {!result.success && !result.skipped && result.errorMessage && (
            <Text className="text-danger text-xs">({result.errorMessage})</Text>
          )}
        </View>
      ))}
    </View>
